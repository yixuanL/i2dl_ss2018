...
def affine_forward(x, w, b):
    ...
    new_x = x.reshape(n,np.prod(x.shape[1:]))
    new_b = np.tile(b,(n,1))
...
def batchnorm_backward(dout, cache):
    ...
    m = out.shape[0]
    dx_norm = dout * gamma
    dsqrtvar = np.sum(dx_norm*x_minus_mean*(-0.5)*(ivar**(3)),axis=0)
    dmean = np.sum(dx_norm*(-ivar), axis=0) + dsqrtvar*(np.sum(-2*x_minus_mean, axis = 0)/m)
    dx = dx_norm*(ivar) + dsqrtvar*(2*x_minus_mean/m) + dmean/m
